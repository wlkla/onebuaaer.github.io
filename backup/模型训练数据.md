...
GPU卡数 | batch-size | learning-rate | train-epochs | 训练模式 | 模型
-- | -- | -- | -- | -- | --
4 | 4 | 0.001 | 3 | SFT-LoRA | qwen/qwen-14b
...
- 报错原因：本次训练日志文件未丢失，PaddlePaddle抛出`ValueError`，但不清楚具体原因，决定放弃该参数。
...
- 报错原因：训练过程中出现空向量。结合之前的训练，总结出一个经验：qwen模型学习率最好小于0.01，否则模型训练会报错。